...
配色は `slides.yaml` の `theme:` ブロックで一元指定し、`lib/theme.ts` が解決して `C`（カラー）/ `FONT*`（フォント）として全 `pages/*.ts` に供給する。
...
`slides.yaml` 先頭の `theme:` ブロックに書く。解決の優先順は **preset → primary(シード) → デフォルト(blue)**、そのうえで**フィールド単位の上書き**が最優先で乗る。

### 1. プリセットを選ぶ（最も手軽）

```yaml
theme:
  preset: blue        # blue / warm / forest / slate
```

| preset | 雰囲気 | Primary | Accent |
|--------|--------|---------|--------|
| `blue`   | 既定・信頼感のある青 | `#1E88E5` | `#FF9800` |
| `warm`   | コーラル/アンバーの暖色 | `#E5533D` | `#FFB300` |
| `forest` | 落ち着いたグリーン | `#2E7D32` | `#F9A825` |
| `slate`  | モノクロ＋1アクセント | `#37474F` | `#FF7043` |
