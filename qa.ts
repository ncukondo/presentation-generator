...
async function reviewOne(filename: string): Promise<SlideReport> {
  ...
  const prompt = [
    ...
    `Be strict. Flag overflow, overlap, unnatural wrap, low contrast, empty regions, color misuse, decorative icons, placeholder text, broken citations, over-trimmed content (text cut so short the slide no longer conveys a complete idea), and redundant elements (a subtitle, banner, note, caption or description that merely restates another element on the slide — recommend removing the element entirely), speculative or opinion wording on the slide (「〜と思われる」「〜だろう」「〜すべき」 etc. — slides state facts, interpretation goes in the narration), and titles or headings that are sentences, questions or slogans instead of concise noun phrases. Fitting the layout never justifies dropping information the audience needs; recommend a layout change instead.`,
  ].join("\n");
