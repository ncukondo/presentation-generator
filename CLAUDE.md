...
- **レイアウトは「内容 → 理想像 → 既存流用 or 新設」の順で決める。** まず内容を検討し、
  それを聴衆に伝えるのにふさわしい見せ方の理想像を考える。当てはまる既存 layout があれば流用、
  無ければ新設または変更する（`docs/design-principles.md` 4章）。一覧から近いものを選んで
  内容を合わせる、という順序にしない。
...
想定聴衆は**医学系学会発表の慣例に慣れた人**。その慣例に合わせて書く（`docs/design-principles.md` 10章）。

- **タイトルは簡潔な名詞句。** 文（「〜である」「〜が必要だ」）や問いかけ、キャッチコピーにしない。
- **スライドには事実だけを書く。憶測・推測・意見は書かない。** 「〜と思われる」「〜だろう」
...
- `layout` — レンダラ名（`title` / `agenda` / `bullets` / `section` / `number-cards` /
  ...
  ※ 決め方は「内容を検討 → 伝えるのにふさわしい理想像 → 既存にあれば流用、無ければ手順3で新設・変更」。
- `title` — タイトル。
