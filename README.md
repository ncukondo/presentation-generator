...
（`lib/render.ts`）がそれを描く。スライドの**追加・編集・並べ替え・複製は YAML 編集だけで完結**し、
発表者がコードを触らずに内容を回せる。
...
- **テーマ（配色）の切替** — `slides.yaml` の `theme:` ブロックで配色を一元指定（preset / シード色 / 個別上書き）。`lib/render.ts` は役割名（`C.xxx`）参照なので無改修
